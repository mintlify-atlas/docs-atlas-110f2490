...
- **Reset the polygon** — Click the **Clear** button below the map to remove all vertices and start over. You can customize the button label with `->clearButtonLabel('Reset')`.
- **Navigate to the polygon** — Click anywhere inside a drawn polygon to pan and zoom the map so the full polygon is visible.

<Tip>
  For the full list of available methods on both `DrawableMap` and `ViewableMap` — including `polygons()`, `titles()`, `mode()`, and `clearButtonLabel()` — see the [DrawableMap component reference](/components/drawable-map).
</Tip>
