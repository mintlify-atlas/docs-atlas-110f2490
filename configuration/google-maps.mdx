...
The package service provider calls `FilamentAsset::registerScriptData()` during the `packageBooted` phase. This injects the API key into Filament's shared JavaScript data bag, making it available to the Alpine.js map component as `window.filamentData.api.key` at runtime.
...
The `ViewableMap` Blade template also references the key directly via `config('filament-drawable-map.providers.google.key')` when constructing the Google Maps script URL, so the key must be present in the published config or the environment variable must resolve correctly through the config file.
