...
<ParamField path="providers.google.key" type="string|null" default="null">
  Your Google Maps JavaScript API key. The package reads this value from the `GOOGLE_MAPS_API_KEY` environment variable by default. It is injected into Filament's shared JavaScript data bag via `FilamentAsset::registerScriptData()` and is also referenced directly in the `ViewableMap` Blade template when constructing the Google Maps script URL. Set this to `null` (or leave the env var unset) to disable map rendering.
</ParamField>
